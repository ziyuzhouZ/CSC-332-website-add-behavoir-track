    ...
    if (wasLoggedIn) {
        try {
            ...
            if (userData) {
                ...
                // 确保行为跟踪器在用户登录状态确认后初始化
                if (typeof window.behaviorTracker === 'undefined') {
                    console.log('初始化行为跟踪器');
                    window.behaviorTracker = new BehaviorTracker();
                } else if (typeof window.reinitializeBehaviorTracker === 'function') {
                    console.log('重新初始化行为跟踪器');
